    ...
    if len(new_tweets) > 0:
        ...
        while (len(new_tweets) > 0) & (year >= stop_year):
            ...
            try:
                ...
                print "...%s tweets downloaded so far, latest tweet collected is within %s." % (len(all_tweets), year)
...
if __name__ == '__main__':
    ...
    dataFrame = pandas.read_csv('labelled_MPs.csv',
                     header=None,
                     names=['name', 'screen_name', 'party', 'label'])
    ...
    for (screen_name, name, label) in itertools.izip(iterScreenName, iterName, iterLabel):
